...
// Sử dụng exec để thực hiện lệnh dòng lệnh
...
function replacePathSlashes(path: string) {
...
function getJoinedStringFromArray(arr: string[]) {
  return arr.join(' ')
}

export const checkVideoHasAudio = async (filePath: string) => {
  const stdout = await execCommand(
    `ffprobe ${getJoinedStringFromArray([
      '-v',
      'error',
      '-select_streams',
      'a:0',
...
const encodeMax720 = async ({
  ...
  args.push(
    ...
    replacePathSlashes(outputSegmentPath),
    replacePathSlashes(outputPath)
  )

  await execCommand(`ffmpeg ${getJoinedStringFromArray(args)}`)
  return true
}
...
export const encodeHLSWithMultipleVideoStreams = async (inputPath: string) => {
  ...
  console.log('🚀 ~ encodeHLSWithMultipleVideoStreams ~ resolution:', resolution)
  console.log('🚀 ~ encodeHLSWithMultipleVideoStreams ~ bitrate:', bitrate)
